...
def scrape_IMS_weekly():
    ...
    for day in list(data['data'].values()):
        for hour in list(day["hourly"].values()):
            temp = {"forcast_time": str(hour["forcast_time"]), "weather_code": str(hour["weather_code"]), "temperature": str(hour["temperature"]), "weather": ""}
            try:
    ...
    for single_current_data in current_data:
        current_time = datetime.strptime(single_current_data['forcast_time'][:19], '%Y-%m-%d %H:%M:%S')
        current_item = Weekly.objects.filter(forcast_time=current_time)
        if current_item.exists():
            ...
            Weekly.objects.create(forcast_time=current_time, weather_code=str(single_current_data['weather_code']), weather=str(single_current_data['weather']), Temperature=str(single_current_data['temperature']))
